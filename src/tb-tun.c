...
struct Threadargs {
    ...
    int remote_ip;
    /**
     * 0 for 6to4, 1 for tunnelbroker, 2 for isatap
     */
    int tun_mode;
};
...
void printip(int ipaddr) {
    printf("%d.%d.%d.%d", 0x000000ff & ipaddr, (0x0000ff00 & ipaddr) >> 8, (0x00ff0000 & ipaddr) >> 16,
           (0xff000000 & ipaddr) >> 24);
}
...
void s2t_thread(void *s2targs) {
    ...
    while (1) {
        ...
        if (tun_mode == 1) {
            /**
             * Only accept package with source IPv4 addr the same as tunnel server's
             */
             ...
             */
            if ((*(int *) (&bufsock[12]) != remote_ip) && ((*(int *) (&bufsock[(leniphead + 16)]) != 0xfe5e0000) ||
                                                           (*(int *) (&bufsock[12]) !=
                                                            *(int *) (&bufsock[leniphead + 20])))) {
                printf("s2t: Drop package from source IPv4 ");
                printip(*(int *) (&bufsock[12]));
                printf(" do not corresponds its IPv6 address.\r\n");
                continue;
            }
        } else if ((tun_mode == 0) && (*(int *) (&bufsock[12]) != remote_ip)) {
            /**
    ...
    while (1) {
        ...
        switch (tun_mode) {
            ...
            case 2:
                 ...
                 */
                if ((*(int *) (&buftun[(sizeof(struct tun_pi) + 24)]) ==
                     *(int *) (&buftun[(sizeof(struct tun_pi) + 8)])) &&
                    (*(int *) (&buftun[(sizeof(struct tun_pi) + 28)]) ==
                     *(int *) (&buftun[(sizeof(struct tun_pi) + 12)])) &&
                    (*(int *) (&buftun[(sizeof(struct tun_pi) + 32)]) ==
                     *(int *) (&buftun[(sizeof(struct tun_pi) + 16)])) &&
                    (*(int *) (&buftun[(sizeof(struct tun_pi) + 16)]) == 0xfe5e0000)) {
                    remoteaddr.sin_addr.s_addr = *(int *) (&buftun[(sizeof(struct tun_pi) + 36)]);
                }
                break;
        }
        res = sendto(sockv6, &buftun[sizeof(struct tun_pi)], ret, 0, (struct sockaddr *) &remoteaddr,
...
int main(int argc, char *argv[]) {
    ...
    if (localaddr.sin_addr.s_addr != -1) {
        ...
        */
        printf("Do not bind local IPv4 address, using default.\r\n");
    }
